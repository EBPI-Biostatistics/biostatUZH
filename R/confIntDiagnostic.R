...
#' @param names Vector of length 2. Names of first and second diagnostic tests.
#' Default to c(1, 2).
...
#' @return A data.frame containing the estimated sensitivity, specificity,
...
confIntIndependentDiagnostic <-  function (tp, fp, tn, fn, names = c(1, 2), conf.level = 0.95, adjust = FALSE)
{
  ...
  estimate1 <- resultA[1:4, 2]
  estimate2 <- resultB[1:4, 2]
...
#' results.
#' @param names Vector of length 2. Names of first and second diagnostic tests.
#' First name refers to the test in the columns; second name refers to the test
#' in the rows.
#' Default to c(1, 2).
#' @param conf.level Confidence level for confidence interval. Default is 0.95.
#' @param adjust Logical of length one indicating whether to compute adjusted
#' CIs or not. Default is FALSE.
#' @return A data.frame containing the estimated sensitivity, specificity,
#' LR+ and LR- in the two tests, as well as the ratio (first versus second test or
#' equivalently column test versus row test)
#' with the corresponding confidence interval.
#' @author Leonhard Held
                                    ...
                                    conf.level = 0.95, adjust = FALSE)
{
  ...
  estimate1 <- c(sens1, spec1, LR_Plus1, LR_Minus1)
  estimate2 <- c(sens2, spec2, LR_Plus2, LR_Minus2)
  ratio <- estimate1/estimate2
  ...
  res <- data.frame(matrix(data = NA, ncol = 6, nrow = 4))
