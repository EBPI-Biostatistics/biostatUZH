...
#' @param alpha Significance level with default 0.05.
...
#' @param non.inf.margin Non-inferiority margin. Default is 0 and implies a superiority trial.
#' @param alternative "two.sided" (default) or "one.sided". Specifies whether the
#' alternative hypothesis is one- or two-sided.
#' @return If \code{power} is specified, the function returns the required total number of events.
#' If \code{n.events} is specified, the function returns the power.
...
#' @examples
#' 
#' survEvents(HR = 0.65, alpha = 0.05, power = 0.9, alloc.ratio = 1,
#'            non.inf.margin = 0, alternative = "two.sided")
#'
#' survEvents(HR = 0.65, alpha = 0.05, power = 0.9, alloc.ratio = 1,
#'            non.inf.margin = .5, alternative = "one.sided")
#' 
#' survEvents(HR = 0.65, alpha = 0.05, n.events = 35, alloc.ratio = 1,
#'            non.inf.margin = 0, alternative = "two.sided")
#' 
#' survEvents(HR = 0.65, alpha = 0.05, n.events = 35, alloc.ratio = 1,
...
#' 
#' @export
survEvents <- function(HR, alpha = 0.05, power = NULL, n.events = NULL,
    ...
    logHR <- log(HR)
    z_alpha <- if(alternative == "one.sided") qnorm(alpha) else qnorm(alpha / 2)
    ...
    if(is.null(n.events)){
        ...
        ## superiority case
        power <- 1 - stats::pnorm((logHR - non.inf.margin) * sqrt(n.events * p.factor) - z_alpha)
        return(power)
    }
...
NumEvents <- function(HR, sig.level=0.05, power=NULL, n.events=NULL,
    ...
    if(!is.null(n.events)){
        ## superiority case
        if(type %in% c("superiority","sup")){
            power <- 1 - stats::pnorm(logHR*sqrt(n.events*p.factor)-z_alpha)
            return(power)
        }
        ## non-inferiority case
        if(type %in% c("non-inferiority","noninf")){
