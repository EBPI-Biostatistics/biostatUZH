...
#' 
#' In survival analysis, often some quantiles (e.g., the median) of an
#' estimated cumulative incidence function are of interest. This function
#' computes quantiles of cumulative incidence functions using
#' \code{cuminc} of the package \pkg{cmprsk}.
#'  
...
#' @return data.frame with quantiles estimates for each event-group combination.
...
{
    ## 'time', 'event', 'group' are checked in cmprsk::cuminc()
    ...
    est0 <- cmprsk::timepoints(cmprsk::cuminc(time, event, group = group,
                                              rho = 0, cencode = 0), times = mean(time))$est
    res <- matrix(NA, nrow = nrow(est0), ncol = 1)
    rownames(res) <- rownames(est0)
    colnames(res) <- paste(quant, "-quantile", sep = "")
    
    for (r in 1:nrow(est0)){
        tmp <- function(x, time, event, group){
            cmprsk::timepoints(cmprsk::cuminc(time, event, group = group, rho = 0, cencode = 0),
                               times = x)$est[r, 1] - quant
        }
