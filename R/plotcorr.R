                      ...
                      cex.lab = par("cex.lab"), cex = 0.75*par("cex"), mar = 0.1 + c(2,2,4,2), ...)
{
  ...
  if (is.null(corr) || !is.matrix(corr)
    ...
    stop("Matrix is not a valid correlation matrix")
  ...
  graphics::text(cols, rep(length(rows) + 1, length(cols)), labels = collabs[cols], 
                 srt = 90, adj = 0, cex = cex.lab)
  graphics::mtext(xlab,1,0)
  graphics::mtext(ylab,2,0) 
  mat <- diag(c(1, 1))
  plotcorrInternal <- function()
  {
    ...
    if (panel == "ellipse") {
      ...
      graphics::polygon(ell, col = col[i, j])
      if (outline) lines(ell)
    } else {
      graphics::text(j + 0.3, length(rows) + 1 - i, round(10 * corr[i, j], 0),
                     adj = 1, cex = cex)
    }
  }
